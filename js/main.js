...
function outputjoin(message){
    ...
    div.innerHTML=`<p class="meta">${message.text}</p>`;

    document.querySelector('.chat-messages').appendChild(div);
    }
function outputmsg(massage){
    const div =document.createElement('div');
    div.classList.add('missage');
    div.innerHTML=`<p class="meta">${massage.username}</p>
    <p class="text">
        ${massage.text}
    </p>`;
    document.querySelector('.chat-messages').appendChild(div);
}
function inputmsg(massage){
    const div =document.createElement('div');
